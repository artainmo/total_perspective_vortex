...
def main(algo, subject, task):
    ...
    print(f'{"Dataset/subject and task:": <55}{subject} and {task}')
    print(f'{"Classifier algorith:": <55}{classifier}')
    print(f'{f"average score over {nb_tests} experiments of training set:": <55}{sum(train_scores)/len(train_scores)}')
    print(f'{f"average score over {nb_tests} experiments of test set:": <55}{sum(test_scores)/len(test_scores)}')
    print(f'{"Total execution time in seconds:": <55}{"NA" if g_skip else end_time-start_time}')
    _continue()
